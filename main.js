...
function initCounters() {
  // Target both old .stat-num and new .hss-num elements
...
function animateCounter(el) {
  ...
  const decimalVal = el.dataset.decimal || null;
  ...
  function step(now) {
    ...
    if (decimalVal) {
      const floatTarget = parseFloat(decimalVal);
      countEl.textContent = (ease * floatTarget).toFixed(1);
    } else {
      countEl.textContent = Math.floor(ease * target);
    }

    if (progress < 1) {
      requestAnimationFrame(step);
    } else {
      countEl.textContent = decimalVal || target;
    }
  }
...
🌴 Ready to explore Goa with RoyalGoaRide! 🏖️`
        );
      }
